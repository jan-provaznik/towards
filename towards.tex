...
\begin{figure}[h]
  ...
  \end{center}
  \caption{
    Thresholds of the tolerable loss in preparation of certifiable genuine $3$~(black), $4$~(red), and $5$-PQnG (blue) states. Their values are determined using the same methodology as the results visualised in \figref{f-res-45} where the minimal viable probability of success is limited to $10^{-5}$.
    %
...
- loss and low detection efficiency are an unavoidable part of daily life in quantum optics
- it hinders preparation of the more exotic quantum non-Gaussian states
- in this sense the individual photonic states are both resources in construction of these states and can be also exploited to indicate the quality of the experimental control as they are the bare minimum requirement
- special criteria more robust than fidelity have to be used to certify the prepared states as loss mangles the produced state at every point of its life; higher-ordered components are introduced during heralding in its preparation, lower-ordered contributions appear due to inefficient detection in its characterization
- we used the hierarchical criteria based on stellar rank to show that the resulting state could not be produced from a lower photonic state through Gaussian operations
- we have shown the maximal tolerable loss for experimental realization of individual photonic states of 4 and 5 photons which have yet to be done
- this might serve as an encouragement to our fellow experimental practitioners
