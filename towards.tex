...
\end{equation}
%
This model of the state preparation employs an ideal PNR detector in the heralding stage of the circuit. The mathematical description can be extended to include the experimentally available cascaded avalanche photodiode~(CAP) detectors~\cite{paul1996,provaznik2020} that approximate actual PNR detectors. The incoming signal is divided equally between $n$ independent avalanche detectors comprising the cascade. The detection events when $m$ out of the $n$ constituent detectors become triggered, that is, register any positive number of photons, are associated with the elements~\cite{paul1996}
%
\begin{equation}
  \begin{gathered}
  \hat{\Pi}_{m}^{n} 
...
%
\begin{equation}
  P'_{m} = \sum_{k} w(k, m, n) P_{k}
\end{equation}
\begin{equation}
  \hatrho'_{\vert_{m}} = \frac{1}{P'_{m}} \sum_{k} w(k, m, n) P_{k} \hatrho_{\vert_{k}}
\end{equation}
