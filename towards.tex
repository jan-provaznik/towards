...
The hierarchical criteria \cite{lachman2019} can be used to determine whether a state $\hatrho$ belongs to specific $n$-photon hierarchy by examining a relationship between its first $n$ Fock state contributions,
\begin{equation}
  x_{n} 
    = 1 - \sum_{k = 0}^{n - 1} 
      \braket{k | \hatrho | k}
...
\end{equation}
%
The state is a genuine $n$-photon quantum non-Gaussian state when
%
\begin{equation}
  {y_{n} \geq F_{n} (x_{n})} \Qd
\end{equation}
%
The threshold function $F_{n}(x)$ can be obtained through numerical optimization \cite{lachman2019,fiurasek2022}.
