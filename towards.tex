...
%
\begin{equation}
  P_{(m)}^{(n)} = \sum_{k} w(k, m, n) P_{(k)}
\end{equation}
%
as the elements \eqref{e-capd} of CAP detectors are diagonal in Fock basis. Similarly, the resulting density matrix remains diagonal, its elements given by
\begin{equation}
  \hatrho_{(m)}^{(n)} =
    \frac{1}{P_{(m)}^{(n)}} 
    \sum_{k} w(k, m, n) 
